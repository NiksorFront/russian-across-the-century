<script setup>
  ...
  // Логика для переключения слайдов (для стрелок)
  const nextSlide = () => {
    ...
    <div class="flex items-center blur-border">
      ...
      >
        ...
        >
          ...
          <img v-if="item.image" :src="item.image" alt="Image" class="w-10/12 h-fit object-cover aspect-for-img rounded-[clamp(10px,0.5vw,40px)] mx-auto" />
          <button 
            v-if="item.modalText"
            @click="openModal(item)"
            class="w-[clamp(39px,7vw,57px)] h-[clamp(39px,7vw,57px)] bg-[#101fb3] absolute -top-2.5 -right-2.5 rounded-full rotate-45" >
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="5975 5062.5888671875 59 59"> <path fill="#0C27AF" fill-opacity="1" stroke="" stroke-opacity="1" stroke-width="1" fill-rule="evenodd" id="tSvg12877558042" d="M 6004.499825306921 5063.588845863561 C 6004.499825306921 5063.588845863561 C 6004.499825306921 6032.999825306921 5063.588845863561 C 6032.999825306921 C 6004.499825306921 Z"></path> </svg>
          </button>
        </li>
        <!-- Мини костыль -->
